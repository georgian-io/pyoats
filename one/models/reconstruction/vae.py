...
class VAEModel(PyODModel):
    ...
    def fit(self, train_data, **kwargs):
        ...
        if not self.params.get("encoder_neurons"):
            self.params["encoder_neurons"] = [n_feat*self.window, n_feat*self.window//2, n_feat*self.window//4]
        if not self.params.get("decoder_neurons"):
            self.params["decoder_neurons"] = [n_feat*self.window//4, n_feat*self.window//2, n_feat*self.window]
