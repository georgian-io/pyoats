...
class FluxEV2000:
    ...
    def load_initial(self, data):
        if data.size <= 2*self.s:
            raise ValueError(f"Expects initial series of length > max(2*s, p) == {max(2*self.s, self.p)}, passed in {data.size}")
        t = time.perf_counter()
    ...
    def step_predict(self, x_t) -> int:
        ...
        if self.n % self.bw_reset_every == 0:
            ...
            self.bw = SPOTMoM.fit_kde_bw(self.F_mem)
        
        
        self._step_train(x_t)
        new_p_thres = SPOTMoM.get_tail_threshold(self.F_mem, self.level, samples=100, bw=self.bw)
    ...
    def _spot_initialize(self, S):
        ...
        # print(S[S>0])
        self.bw = SPOTMoM.fit_kde_bw(S)
        self.percentile_thres = SPOTMoM.get_tail_threshold(S, self.level, bw=self.bw)
        ...
        I = SPOTMoM.get_peaks_idx(S, self.percentile_thres)
        
        if self.init_cutoff < 1:
            bw = SPOTMoM.fit_kde_bw(Y)
            cutoff = SPOTMoM.get_tail_threshold(Y, self.init_cutoff, bw=bw)
            Y = Y[Y<cutoff]
...
class SPOTMoM:
    ...
    @classmethod
    def get_tail_threshold(cls, data, level, samples=1000, bw=0.5, kernel="gaussian", rtol=0.02) -> float:
        # via KDE
...
class FluxEVModel(Model):
    ...
    def fit(self, data, **kwargs):
        ...
        self.model = FluxEV2000(self.window, q=self.q, level=self.level, p=self.window_smoothing, **self.params)
